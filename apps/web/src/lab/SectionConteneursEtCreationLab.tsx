...
type PropsSectionCreationLab = {
  imageCreation: string;
  setImageCreation: (v: string) => void;
  nomCreation: string;
  setNomCreation: (v: string) => void;
  surCreer: () => void;
};

/** Formulaire minimal de création de conteneur (image et nom optionnel). */
export function SectionCreationConteneurLab({
  imageCreation,
  setImageCreation,
  nomCreation,
  setNomCreation,
  surCreer,
}: PropsSectionCreationLab) {
  return (
    <section style={styleBlocLab}>
      <h2 style={{ fontSize: "1rem", marginTop: 0 }}>Créer un conteneur</h2>
      <label style={{ display: "block", maxWidth: 400, marginBottom: 8 }}>
        Image
        <input
          value={imageCreation}
          onChange={(e) => setImageCreation(e.target.value)}
